# JeeSite-lite 企业信息化快速开发平台

## 平台简介

JeeSite-lite是基于Jeesite，整合多个优秀的开源项目，高度整合封装而成的高效，高性能，强安全性的**开源**Java EE快速开发平台。
是您快速完成项目的最佳基础平台解决方案，是您想学习Java平台的最佳学习案例，还是接私活的最佳助手。

JeeSite-lite是在Spring Framework基础上搭建的一个Java基础开发平台，以Spring MVC为模型视图控制器，MyBatis为数据访问层，
Apache Shiro为权限授权层，Ehcahe对常用数据进行缓存。是JavaEE界的最佳整合。

JeeSite-lite主要定位于企业信息化领域，已内置企业信息化系统的基础功能和高效的**代码生成**工具，
包括：系统权限组件、数据权限组件、数据字典组件、核心工具组件、视图操作组件、代码生成等。
前端界面采用了零门槛开箱即用的前端UI解决方案[Layui](http://www.layui.com)代替了原本的比较陈旧的Bootstrap2。
...
如果你使用了本基础框架，就可以很高效的快速开发出，优秀的信息管理系统。
...
* 持久层框架：MyBatis Plus 2.1
* 数据库连接池：Alibaba Druid 1.0
* 缓存框架：Ehcache 2.6、Redis
* 日志管理：SLF4J 1.7、Log4j
* 工具类：Apache Commons、Jackson 2.7、Xstream 1.4、Guava 20.0、POI 3.14
...
* 数据库支持：目前仅提供MySql、Oracle和SqlServer数据库的支持，但不限于数据库，平台留有其它数据库支持接口，
你可以很方便的更改为其它数据库，如：PostgreSQL、H2等
...
* <http://jeesitelite.ngrok.cc>  &nbsp; 用户名：thinkgem &nbsp; 密码：admin
...
4. 运行db\init-db.bat脚本(mysql可直接导入db目录下jeesite-lite.sql)，即可导入表结构及演示数据(linux操作系统：在控制台中切换至项目根目录，运行命令：mvn antrun:run -Pinit-db)
