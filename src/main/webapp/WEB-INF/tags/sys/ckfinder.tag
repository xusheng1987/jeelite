...
<ol id="${input}Preview" style="padding:0px;margin:0 0 10px 25px"></ol><c:if test="${!readonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="layui-btn layui-btn-normal">${selectMultiple?'添加':'选择'}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="layui-btn layui-btn-danger">清除</a></c:if>
